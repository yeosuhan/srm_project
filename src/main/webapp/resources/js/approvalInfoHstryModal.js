				...
				success : function(result) {
					...
					if (auth == 'ROLE_ADMIN' && AhstryType == 'A') {
						if (ArqstrId != autuId && AhstryStts == 'I') {
							...
							$('#footDiv')
									.html(
											'<button type="submit" form="historyDetailForm" formmethod="post" class="btn btn-info center" style="font-weight: bold;font-size: 20px">등록</button>');
						...
						if (ArqstrId == autuId && AhstryStts == 'I') {
							...
							$('.mheadDiv')
									.html(
											'<input type="button" class="btn btn-info btn-sm" style="line-height: 8px;sfont-weight: bold;font-size: 20px" value="수정" data-toggle="modal" data-target="#modifyHistoryModal">'
													+ '<input type="hidden" class="AhstryId" name="hstryId" value="">');
							$('#HstrySttsDiv2')
									.html(
											'<label class="mr-3"><input id="AHstryStts1" type="radio" name="hstryStts" value="Y" onclick="return(false);">승인</label>'
