...
<div class="pagination">
	<c:if test="${pager.startPageNo -1 > 0 }">
		<a class="pagination-newer" href="#"
			onclick="mytodoHstryPaging(1); event.preventDefault()">PREV</a>
	</c:if>
	<c:if test="${pager.startPageNo -1 <= 0 }">
	...
	<c:forEach var="num"
			...
			<c:if test="${pager.pageNo == num }">
				<a class="pagination-active" href="#"
					onclick="mytodoHstryPaging(${num}); event.preventDefault()">${num}</a>
			</c:if>
			<c:if test="${pager.pageNo != num }">
				<a href="#" onclick="mytodoHstryPaging(${num}); event.preventDefault()">${num}</a>
			</c:if>
		</c:forEach>
	</span>
