		...
		<div class="pagination">
			<c:if test="${qPager.startPageNo -1 > 0 }">
				<a class="pagination-newer"
					href="/board/list?type=qna&page=${qPager.startPageNo - 1}">PREV</a>
			</c:if>
			<c:if test="${qPager.startPageNo -1 <= 0 }">
			...
			<span class="pagination-inner"> <c:forEach var="num"
					...
					<c:if test="${qPager.pageNo == num }">
						<a class="pagination-active" href="/board/list?type=qna&page=${num}">${num}</a>
					</c:if>
					<c:if test="${qPager.pageNo != num }">
						<a href="/board/list?type=qna&page=${num}">${num}</a>
					</c:if>
				</c:forEach>
			</span>
			<c:if test="${qPager.endPageNo < qPager.totalPageNo }">
				<a class="pagination-older"
					href="/board/list?type=qna&page=${pager.endPageNo + 1}">NEXT</a>
			</c:if>
			<c:if test="${qPager.endPageNo >= qPager.totalPageNo }">
